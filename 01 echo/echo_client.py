...
def main():
    ...
    parser.add_argument('-f', '--filename',
                        help='File to Echo', required=False)
    parser.add_argument('-d', '--destination',
                        help='Destination IP Host', required=True)
    parser.add_argument('-p', '--remoteport',
                        help='remote sock352 UDP port', required=False)
    parser.add_argument('-l', '--localport',
                        help='local sock352 UDP port', required=True)
    ...
    for i, sent_byte in enumerate(digest_sent):
        ...
        if (sent_byte != recv_byte):
            print("echo_client: digest failed at byte %d diff: %c %c " %
